...
게이트웨이가 없는 실습실 스위치의 사설 IPv4와 랜선 직접 연결에서 사용하는
`169.254.x.x` IPv4 링크 로컬 주소도 학생 주소로 표시합니다. IPv6 전용망은 지원하지 않습니다.
실제 Wi-Fi 또는 Ethernet 주소가 있으면 VMware·VPN 같은 가상 어댑터 주소는 숨깁니다.
실제 접속 주소가 여러 개면 어댑터 이름과 IP를 함께 표시해 복사할 주소를 고를 수 있습니다.
...
- **따라가기**를 켜면 교수님이 보고 있는 파일과 줄로 이동합니다. 교수님 줄이 화면에서
  완전히 벗어나도록 직접 스크롤할 때만 꺼지며, 다른 파일을 직접 열어도 상태는 유지됩니다.
  `▶▶`를 누르면 교수님 파일로 다시 이동합니다.
- 줄 번호를 클릭하거나 드래그해 줄 전체를 선택합니다. Shift+클릭으로 범위를 늘릴 수 있습니다.
- 터치 화면에서는 줄 번호를 코드박스 가장자리로 드래그하면 자동 스크롤됩니다.
  거터와 코드 영역에 손가락을 하나씩 두면 어느 쪽을 먼저 눌렀든 두 번째 손가락으로
  여러 번 스크롤하면서 범위를 늘릴 수 있고, 가장자리에서는 자동 스크롤됩니다.
  코드 영역을 길게 누른 뒤 가장자리로 드래그해도 줄 범위가 자동으로 늘어납니다.
...
1.4.4는 한국어, 영어, 일본어, 중국어(간체), 중국어(번체), 스페인어, 프랑스어, 독일어,
브라질 포르투갈어, 러시아어, 힌디어, 아랍어, 인도네시아어, 베트남어, 튀르키예어, 태국어를
포함합니다.
